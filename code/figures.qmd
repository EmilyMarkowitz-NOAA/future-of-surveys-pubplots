...
We conducted a literature review using the web of science citation system (search query: <https://www.webofscience.com/wos/woscc/summary/7f060455-0bd6-4643-bd2f-2de3ceedb0b2-f52da389/relevance/1>). To be included in this analysis, each paper had to contain a long-term fisheries survey or pilot study of a new study Our search returned `r format(x = nrow(dat), big.mark = ",")` entries, of which contained `r format(x = dat[dat$include == "DUPLICATE" & !is.na(dat$include),], big.mark = ",")` duplicate results and `r format(x = dat[dat$include == "NO" & !is.na(dat$include),], big.mark = ",")` irrelevant results, leaving `r format(x = dat[dat$include == "YES" & !is.na(dat$include),], big.mark = ",")` relevant articles to categorize. The following categories were used in this analysis: 
  ...
  **Gear Type**: 
  `r paste0(unique(dat_descr$gear_type), collapse = ", ")`

**Research topic**: 
  `r paste0(unique(dat_descr$research_field), collapse = ", ")`

**Location**: 
  `r paste0(unique(dat_descr$location), collapse = ", ")`

**Survey**: 
  `r paste0(unique(dat_descr$survey), collapse = ", ")`

**Organism focus**: 
  `r paste0(unique(dat_descr$organism), collapse = ", ")`
...
#| tbl-cap: "Plot of frequency and year, by research field. "
...
#| tbl-cap: "Plot of frequency and year, by gear type. "

column0 = "gear_type"
figure_print <- auto_plot_descr(dat1 = dat_descr, 
                          column0 = column0, 
                          other_cutoff = 0, 
                          title0 = "Plot of freqency and year, by gear type")
...
```{r}
#| label: location-line
#| tbl-cap: "Plot of frequency and year, by location. "
...
```{r}
#| label: survey-line
#| tbl-cap: "Plot of frequency and year, by survey "

column0 = "survey"
figure_print <- auto_plot_descr(dat1 = dat_descr, 
                          column0 = column0, 
                          other_cutoff = 0, 
                          title0 = "Plot of freqency and year, by survey")
...
```{r}
#| label: location-line
#| tbl-cap: "Plot of frequency and year, by organism "

# column0 = "organism"
# figure_print <- auto_plot_descr(dat1 = dat_descr, 
#                           column0 = column0, 
#                           other_cutoff = 0, 
#                           title0 = "Plot of freqency and year, by organism")
# 
# knitr::include_graphics(path = here::here("output", paste0("line-", column0,".png")))
...
```{r}
#| label: location-map
#| tbl-cap: "Map of research locations. "
column0 <- "location"
figure_print <- figure_base +
